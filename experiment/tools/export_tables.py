...
def export_metrics_tex(metrics: Dict[str, Any], out_path: Path) -> None:
    ...
    lines.append("\\hline")
    lines.append("Variant & Count & Errors & AvgLatency & p50 & p90 & TaskSucc & Factual & Safety & Consist \\\")
    lines.append("\\hline")
    for v in order:
        if v not in variants:
            continue
        ...
        lines.append(" ".join([row[0], "&", row[1], "&", row[2], "&", row[3], "&", row[4], "&", row[5], "&", row[6], "&", row[7], "&", row[8], "&", row[9], "\\\\"]))
    lines.append("\\hline")
